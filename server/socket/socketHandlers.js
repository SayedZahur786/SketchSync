...
export const socketHandlers = (io) => {
  io.on("connection", (socket) => {
    ...
    socket.on("sticky-note-add", (data) => {
      socket.to(data.roomId).emit("sticky-note-add", data);
    });

    socket.on("sticky-note-update", (data) => {
      socket.to(data.roomId).emit("sticky-note-update", data);
    });

    socket.on("sticky-note-delete", (data) => {
      socket.to(data.roomId).emit("sticky-note-delete", data);
    });

    // Text box events
    socket.on("text-box-add", (data) => {
      socket.to(data.roomId).emit("text-box-add", data);
    });

    socket.on("text-box-update", (data) => {
      socket.to(data.roomId).emit("text-box-update", data);
    });

    socket.on("text-box-delete", (data) => {
      socket.to(data.roomId).emit("text-box-delete", data);
    });
